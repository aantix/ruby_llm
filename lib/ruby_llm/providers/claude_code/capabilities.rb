...
module RubyLLM
  module Providers
    class ClaudeCode
      # Determines capabilities and pricing for Claude Code models
      # Since Claude Code uses the underlying Anthropic models, we can reuse similar capabilities
      module Capabilities
        module_function
        ...
        def determine_max_tokens(model_id)
          case model_id
          when /claude-3-7-sonnet/, /claude-3-5/, /sonnet/, /4-5/ then 8_192
          else 4_096
          end
        end
        ...
        def supports_functions?(model_id)
          model_id.match?(/claude-3|sonnet|haiku|opus/)
        end
        ...
        def model_family(model_id)
          case model_id
          when /claude-3-7-sonnet/, /sonnet-4/, /4-5/  then 'claude-3-7-sonnet'
          ...
          when /sonnet/             then 'claude-3-7-sonnet'
          when /haiku/              then 'claude-3-5-haiku'
          when /opus/               then 'claude-3-opus'
          else 'claude-2'
          end
        end
        ...
        def capabilities_for(model_id)
          ...
          if model_id.match?(/claude-3|sonnet|haiku|opus/)
